...
func main() {
	...
	defer conn.Close()
	client := increment.NewIncrementIdClient(conn)
	log.Print("NewIncrementIdClient Success")
	// 先测试CheckKeyExist功能
	key := "test"
	initial_value := uint64(7)
	checkRet, err := client.CheckIncrKeyExist(context.Background(), &increment.GetIncrIdRequest{Name: key})
	if err != nil {
	...
	if !checkRet.Exist {
		_, err = client.CreateIncrKey(context.Background(),
			&increment.CreateIncrKeyRequest{Name: key, InitialValue: initial_value})
		if err != nil {
			log.Fatalf("CreateIncrKeyRequest Error: %s\n", err.Error())
		}
		log.Print("CreateIncrKeyRequest Success")
	}
	incrRet, err := client.GetIncrId(context.Background(), &increment.GetIncrIdRequest{Name: key})
	if err != nil {
